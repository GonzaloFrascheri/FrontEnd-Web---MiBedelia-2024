...
function FuncionarioPeriodoExamen() {
  ...
  const handleSubmit = async (e) => {
    ...
    const diferenciaDias =
      (diaFin.getTime() - diaInicio.getTime()) / (1000 * 60 * 60 * 24);
    if (diferenciaDias > 21) {
      setError(
        "Entre la fecha de inicio y la fecha de fin no deben haber más de 21 días."
      );
      return;
    }
    ...
    try {
      ...
      const { data, status } = await axios.post(
        "Funcionario/registroPeriodoExamen",
        formData
      );
      if (status === 200) {
        setEstado({
      ...
      setEstado({
        message: error.response
          ...
          : "Error al guardar periodo de examen",
