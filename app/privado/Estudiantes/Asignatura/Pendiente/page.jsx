...
function EstudianteAignaturasPendientes() {
  ...
  useEffect(() => {
    if (userData) {
      const fetchCareers = async () => {
        try {
          const response = await axios.get(`/Estudiante/getCarreraInscripto?idEstudiante=${userData.id}`);
  ...
  const obtenerAsignaturasPendientes = async () => {
    try {
      const response = await axios.get(`/Estudiante/getAsignaturasPendientes?idEstudiante=${userData.id}&idCarrera=${selectedCareer}`);
      const { data } = response;
      return data;
    } catch (error) {
      const { status, data } = error.response;
              ...
              <main>
                ...
                <select value={selectedCareer} onChange={handleCareerChange}>
                  ...
                  {careers.map((career, index) => (
                    <option key={index} value={career.id}>{career.nombre}</option>
                  ))}
                </select>
              </main>
            </div>
